...
public class MrugiAI extends AI {
    ...
    FakeCell findClosest(FakeCell cell, List<FakeCell> list){
    ...
    FakeCell findClosestWhoCanHelp(FakeCell cell, List<FakeCell> list){
    ...
    FakeCell findWithMinBacteria(List<FakeCell> list){
        int min =101;
    ...
    int countIncomingBacteria(FakeCell cell){
        float other=0;
        float mine=0;
        for(FakeBacteria bacteria : bacteriaList)
            if(bacteria.destination==cell) {
    ...
    int countIncomingOpponentBacteria(FakeCell cell){
        float other=0;
        for(FakeBacteria bacteria : bacteriaList)
            if(bacteria.destination==cell) {
                if (bacteria.player != this)
    ...
    int coutIncomingMineBacteria(FakeCell cell){
        float mine=0;
        for(FakeBacteria bacteria : bacteriaList)
            if(bacteria.destination==cell) {
                if (bacteria.player == this)
    ...
    double calculateTime(FakeCell source, FakeCell destination) {
    ...
    boolean willBeMine(FakeCell cell) {
        int incomingOpp = countIncomingOpponentBacteria(cell);
        int incomingMine = coutIncomingMineBacteria(cell);
        if (cell.player == null)
    ...
    FakeCell findDestination(FakeCell cell) {
    ...
    boolean isWorthAttacking(FakeCell from, FakeCell to){
    ...
    boolean canSteal(FakeCell cell, FakeCell target){
        ...
        double calculatedtime = calculateTime(cell,target) + time;
        double endtime = fastestAttacker.endTime;
        double delta = fastestAttacker.startTime - fastestAttacker.endTime;
    ...
    float distanceSquered(FakeCell destination, FakeCell source){
