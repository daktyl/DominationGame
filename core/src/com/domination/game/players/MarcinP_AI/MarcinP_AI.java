...
public class MarcinP_AI extends AI {
    public final List<Cell> friendlyCellList = new ArrayList<Cell>();
    public final List<Cell> hostileCellList = new ArrayList<Cell>();
    public final List<Cell> neutralCellList = new ArrayList<Cell>();
    public final List<Cell> oppositeCellList = new ArrayList<Cell>();
    public final List<Bacteria> friendlyBacteriaList = new ArrayList<Bacteria>();
    public final List<Bacteria> hostileBacteriaList = new ArrayList<Bacteria>();
    private int friendlyPower;
    private float friendlyCenterX;
    private float friendlyCenterY;
    private int hostilePower;
    private float hostileCenterX;
    private float hostileCenterY;
    ...
    public float distanceBetweenCells(Cell cell1, Cell cell2) {
        return (float) Math.sqrt(Math.pow(cell1.fakeCell.centerX - cell2.fakeCell.centerX, 2) + Math.pow(cell1.fakeCell.centerY - cell2.fakeCell.centerY, 2));
    }

    private float distanceBetweenPointAndCell(float x, float y, Cell cell2) {
        return (float) Math.sqrt(Math.pow(x - cell2.fakeCell.centerX, 2) + Math.pow(y - cell2.fakeCell.centerY, 2));
    }

    public double expectedDeltaTimeBeetweenCells(Cell cell1, Cell cell2) {
        return (double) (distanceBetweenCells(cell1, cell2) / 100 * 1000);
    }

    private double expectedDeltaTimeBeetweenPointAndCell(float x, float y, Cell cell2) {
        return (double) (distanceBetweenPointAndCell(x, y, cell2) / 100 * 1000);
    }
